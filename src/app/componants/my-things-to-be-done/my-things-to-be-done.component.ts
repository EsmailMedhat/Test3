...
@Component({
  selector: 'app-my-things-to-be-done',
  templateUrl: './my-things-to-be-done.component.html',
  styleUrls: ['./my-things-to-be-done.component.css']
})
export class MyThingsToBeDoneComponent implements OnInit {
  ...
  getThings() {
    this.userServiceService.getAllMyThings().subscribe({
      next: response => {
  ...
  updateThings(temp: Things) {
    this.userServiceService.updateThing(temp).subscribe({
      next: response => {
        alert(response.msg)
      },
      error: err => {
